...
function IdCardValidate(idCard) {
    /*  consolo.log(idCard);
        consolo.log(typeof idCard);
    */
    idCard = trim(String(idCard).replace(/ /g, "")); //去掉字符串头尾空格   
    if (idCard.length == 15) {
...
function checkIdCard(idCard) {
    if (idCard.length == 18 && IdCardValidate(idCard)) {
        ...
        if (idCard.length == 15 && IdCardValidate(idCard)) {
            flag = "0";
            var newStr = "";
            newStr += idCard.substr(0, 6);
            newStr += "19";
            newStr += idCard.substr(6, 9);
            var tmp = 0;
            for (var i = 0; i < 17; i++) {
                tmp += newStr[i] * Wi[i];
            }
            newStr += Verify[tmp % 11];
            return newStr;
        }
    }
}
